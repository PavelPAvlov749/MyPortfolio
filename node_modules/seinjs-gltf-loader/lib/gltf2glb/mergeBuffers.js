'use strict';
var BUFFER_MAX_BYTE_LENGTH = require('buffer').constants.MAX_LENGTH;
...
function mergeBuffers(gltf, defaultName) {
    ...
    var splitBuffers = (buffersByteLength > mergeBuffers._getBufferMaxByteLength());
    var buffersToMerge = {};
    var mergedNameCount = {};
    ...
    for (var mergedName in buffersToMerge) {
        if (buffersToMerge.hasOwnProperty(mergedName)) {
            ...
            var mergedSource = (buffers.length > 1) ? Buffer.concat(buffers) : buffers[0];
            gltf.buffers[index] = {
                name: mergedName,
