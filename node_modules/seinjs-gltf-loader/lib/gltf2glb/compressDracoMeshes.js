...
function compressDracoMeshes(gltf, options) {
    ...
    var quantizationVolume = dracoOptions.quantizationVolume;
    var explicitQuantization = unifiedQuantization || defined(quantizationVolume);
    ...
    if (defined(quantizationVolume)) {
        ...
        var accessors_1 = gltf.accessors;
        var min_1 = arrayFill(new Array(3), Number.POSITIVE_INFINITY);
        var max_1 = arrayFill(new Array(3), Number.NEGATIVE_INFINITY);
        ForEach.accessorWithSemantic(gltf, 'POSITION', function (accessorId) {
            var accessor = accessors_1[accessorId];
            ...
            for (var j = 0; j < 3; ++j) {
                min_1[j] = Math.min(min_1[j], accessorMin[j]);
                max_1[j] = Math.max(max_1[j], accessorMax[j]);
            }
        });
        positionOrigin = min_1;
        positionRange = Math.max(max_1[0] - min_1[0], max_1[1] - min_1[1], max_1[2] - min_1[2]);
    }
