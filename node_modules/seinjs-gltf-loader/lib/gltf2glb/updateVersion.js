            ...
            if (defined(channels)) {
                ...
                for (var i = 0; i < channelsLength; ++i) {
                    ...
                    if (channel.target.path === 'rotation') {
                        ...
                        var length_1 = accessor.count * componentsLength;
                        var typedArray = ComponentDatatype.createArrayBufferView(componentType, source.buffer, byteOffset, length_1);
...
function objectsToArrays(gltf) {
    ...
    ForEach.material(gltf, function (material) {
        ...
        if (defined(extensions)) {
            var materialsCommon_1 = extensions.KHR_materials_common;
            if (defined(materialsCommon_1)) {
                ForEach.materialValue(materialsCommon_1, function (value, name) {
                    if (typeof value === 'string') {
                        materialsCommon_1.values[name] = {
                            index: globalMapping.textures[value]
                        };
                    }
                });
